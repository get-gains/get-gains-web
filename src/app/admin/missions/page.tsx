...
export default function AdminMissionsPage() {
  ...
  const fetchAll = async () => {
    try {
      const [missionsRes, partnersRes] = await Promise.all([
        apiFetchClient<{ missions: Mission[] }>("/admin/missions?limit=100"),
        apiFetchClient<{ partners: Partner[] }>("/admin/partners?limit=100"),
      ]);
  ...
  const handleDeleteMission = async (id: string) => {
    ...
    await apiFetchClient(`/admin/missions/${id}`, { method: "DELETE" });
    fetchAll();
  };

  const handleDeletePartner = async (id: string) => {
    if (!confirm("Are you sure you want to delete this partner?")) return;
    await apiFetchClient(`/admin/partners/${id}`, { method: "DELETE" });
    fetchAll();
  };
